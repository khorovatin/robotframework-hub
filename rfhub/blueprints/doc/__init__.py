...
def doc_for_library(collection_id, keyword=None) -> str:
    ...
    if collection is not None:
        # Determine whether to load by path or by name
        load_target = collection.get('path') or collection.get('name')

        # Generate a new libdoc object on the fly
        libdoc = LibraryDocumentation(load_target)

        # Pass the object to the HTML renderer
        doc = doc_to_html(libdoc, collection.get('doc_format', 'ROBOT'))
        return flask.render_template("doc.html", doc=doc, keyword=keyword)

    return flask.render_template("doc.html", doc="Collection not found", keyword=None)
